...
# Generate all build targets directly from cargo metadata
$(eval $(shell cargo metadata --format-version=1 | jq -rn \
    --arg outdir '$(DEFUSE_OUT_DIR)' --arg reproducible '$(REPRODUCIBLE)' ' \
    [inputs][0].packages[] | select(.metadata.near.reproducible_build) | \
    ...
    "$$(eval \($$name):; \($$base_cmd) --manifest-path=\($$mp) --out-dir=\($$outdir))", \
    ($$b.variant // {} | to_entries[] | \
     ...
     "$$(eval \($$vname):; \($$vcmd) --manifest-path=\($$mp) --out-dir=\($$outdir)/\(.key))" \
    ) \
    '))
