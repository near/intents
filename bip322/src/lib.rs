...
pub use signature::{Bip322Signature, Bip322Error};
...
impl Payload for SignedBip322Payload {
    ...
    fn hash(&self) -> near_sdk::CryptoHash {
        self.signature.compute_message_hash(&self.message, &self.address)
    }
}
...
impl SignedPayload for SignedBip322Payload {
    ...
    fn verify(&self) -> Option<Self::PublicKey> {
        let message_hash = self.signature.compute_message_hash(&self.message, &self.address);
