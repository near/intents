...
- **`wasm_hash`** *(optional, via `pre_approve`)* — pre-approves a SHA-256 hash so `oc_upload_wasm` can be called immediately in the same transaction as `StateInit`, without a separate `oc_approve` call

Use the [`near-oc`](#near-oc-cli-tool) tool to compute the `StateInit` JSON for a given set of parameters.
...
**`OnChain`** — WASM is stored in the account's contract storage at the given `storage_prefix` key. This is set automatically after `oc_upload_wasm`. The `account` field identifies which contract holds the data, and `storage_prefix` is the storage key (Borsh `Vec<u8>`, base64-encoded in JSON).

**`HttpUrl`** — WASM is available at an HTTPS URL, set manually via `oc_set_location`.
...
### View methods
- `oc_updater_id()` — current updater
- `oc_wasm_hash()` — approved hash (hex)
- `oc_wasm()` — uploaded WASM bytes as base64, or `null`
- `oc_location()` — current `WasmLocation`, or `null`
...
```bash
near view <instance-id> oc_location --networkId testnet
```
...
`oc_wasm()` returns the binary as a base64 string:

```bash
near view <instance-id> oc_wasm --networkId testnet \
  | jq -r '.' \
...
`near-oc` computes the `StateInit` JSON for a new Outlayer Project instance.

### Running

```sh
cargo run -p defuse-outlayer-project --example near-oc
```
...
Usage: near-oc [OPTIONS] --updater-id <AccountId>
...
```bash
cargo run -p defuse-outlayer-project --example near-oc -- --updater-id alice.near
```
```
...
near transaction construct-transaction <updater-id> \
  ...
  data-from-json "$(near-oc \
    --updater-id <updater-id> \
    --approve <wasm-sha256-hex> \
    --quiet)" \
  deposit 0NEAR \
  add-action function-call "oc_upload_wasm" \
...
The `state-init` action deploys the instance (with the hash pre-approved), and the `oc_upload_wasm` function-call action uploads the binary — both atomically in a single transaction.
