...
///
/// assert!(
///     WalletEip712::verify(&wallet.public_key(), &msg, &proof),
...
{
    /// Sign an [`AuthMessage`] and return a proof ready to be wrapped into
    /// a [`SignedAuthMessage`](defuse_wallet_sdk::SignedAuthMessage) for
    /// `w_resolve_auth(purpose, recipient, authorization)`.
    pub async fn sign_auth_msg(&self, msg: &AuthMessage) -> Result<Proof, S::Error> {
