...
struct Fixtures {
    ...
    auth_vectors: Vec<Vector<AuthMessage, Eip712AuthMessage>>,
}
...
fn eip712_wallet_message_vectors() {
    ...
    } in request_vectors
    {
        ...
        assert!(
            WalletEip712::verify(&address, &message, &proof.to_string()),
    ...
    } in auth_vectors
    {
        ...
        assert!(
            WalletEip712::verify_auth(&address, &message, &proof.to_string()),
            "proof of vector '{name}' didn't pass verification",
        );
    }
}
