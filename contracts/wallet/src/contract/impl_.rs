...
#[cfg(all(not(feature = "ed25519"), feature = "webauthn-ed25519"))]
const _: () = {
    use crate::signature::{
...
///
/// Each condition below is mutually exclusive so that exactly one `#[near]`
/// attribute fires, regardless of which combination of features is active.
#[cfg_attr(
    feature = "ed25519",
    near(
...
#[cfg_attr(
    all(not(feature = "ed25519"), feature = "webauthn-ed25519"),
    near(
        contract_state(key = STATE_KEY),
        contract_metadata(
            standard(standard = "wallet",                  version = "1.0.0"),
