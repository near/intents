...
impl PromiseSingle {
    ...
    pub fn estimate_gas(&self) -> NearGas {
        self.actions
            .iter()
            .map(PromiseAction::estimate_gas)
            .fold(NearGas::from_gas(0), NearGas::saturating_add)
    }
...
mod tests {
    ...
    #[rstest]
    #[case(p(0), NearGas::from_gas(0))]
    #[case(
        p(0).function_call(
                FunctionCallAction::new("foo")
                .min_gas(NearGas::from_tgas(123))
        ).function_call(
                FunctionCallAction::new("fbaro")
                .min_gas(NearGas::from_tgas(45))
        ), NearGas::from_tgas(123 + 45)
    )]
    fn estimate_gas(#[case] p: PromiseSingle, #[case] expected: NearGas) {
