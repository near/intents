...
use crate::host::mock;
#[cfg(not(target_family = "wasm"))]
use defuse_outlayer_host::crypto::Ed25519Host;
...
pub fn derive_public_key(path: impl AsRef<str>) -> PublicKey {
    ...
    #[cfg(not(target_family = "wasm"))]
    let raw = mock::HOST
        .with_borrow_mut(|h| h.ed25519_derive_public_key(path.to_string()))
        .expect("host");
...
pub fn sign(path: impl AsRef<str>, msg: impl AsRef<[u8]>) -> Signature {
    ...
    #[cfg(not(target_family = "wasm"))]
    let raw = mock::HOST
        .with_borrow_mut(|h| h.ed25519_sign(path.to_string(), msg.to_vec()))
        .expect("host");
