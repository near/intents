...
use defuse_nep641::Purpose;
use impl_tools::autoimpl;
use near_sdk::{env, near, serde_with::serde_as};

use crate::{DOMAIN_NAME, DOMAIN_SEPARATOR, DOMAIN_VERSION};

/// `keccak256("Authorization(string purpose,string recipient,string payload)")`
const AUTHORIZATION_TYPEHASH: CryptoHash = [
    0x95, 0xff, 0xbe, 0x89, 0x35, 0x4b, 0x1b, 0xc6, 0x0a, 0xbd, 0xe5, 0x1f, 0x8f, 0x1e, 0x5c, 0x18,
    0x88, 0x6c, 0x72, 0x82, 0xbc, 0x55, 0x57, 0xb1, 0x0e, 0x4f, 0x28, 0x45, 0x0c, 0x6f, 0xad, 0x83,
];
...
impl Eip712Authorization {
    ...
    fn struct_hash(&self) -> CryptoHash {
        ...
        buf[32..64].copy_from_slice(&env::keccak256_array(self.purpose.to_string().as_bytes()));
...
mod tests {
    ...
    fn build_auth_typed_data_json(purpose: &str, recipient: &str, payload: &str) -> String {
        let escaped_payload =
            near_sdk::serde_json::to_string(payload).unwrap_or_else(|_| unreachable!());
    ...
    fn test_signed_auth_deserializes() {
        ...
        assert_eq!(signed.message.purpose, Purpose::ProveOwnership);
