use defuse::{contract::Role, extensions::relayer::RelayerKeysExt};
use defuse_sandbox::{
    SigningAccount,
    api::signer::generate_secret_key,
...
async fn relayer_keys() {
    ...
    let new_relayer_signer =
        SigningAccount::new(env.defuse.clone(), generate_secret_key().unwrap());
    let new_relayer_public_key = new_relayer_signer.private_key().public_key();
