...
async fn test_deploy_controller_instance(
    ...
    root
        .deploy_instance(GlobalContractId::AccountId(controller_instance.id().clone()), upgradeable_instance_state.clone())
        .await
    ...
    assert_eq!(mutable_controller_instance.gd_index().await.unwrap(), upgradeable_instance_state.index);
    ...
    assert_eq!(
        mutable_controller_instance.global_contract_id().await.unwrap(),
...
async fn test_deploy_escrow_swap(#[future(awt)] deployer_env: DeployerEnv, unique_index: u32) {
    let root = deployer_env.sandbox.root();
    let alice = root.generate_subaccount("alice", NearToken::from_near(100)).await.unwrap();
    let bob = root.generate_subaccount("bob", NearToken::from_near(100)).await.unwrap();
    let deployer_code_hash_id = deployer_env.deployer_global_id.clone();
    ...
    let upgradable_controller_instance  = root
            ...
            .await
            .unwrap();
    alice.gd_deploy(&upgradable_controller_instance.id(), &DEPLOYER_WASM)
        .await
        .unwrap();
