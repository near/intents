...
use crate::utils::wasms::{DEPLOYER_WASM, DEPLOYER_WITH_USE_ME_WASM};
use defuse_escrow_swap::{ContractStorage, Deadline, OverrideSend, Params};
use defuse_global_deployer::{ERR_UNAUTHORIZED, Storage as DeployerStorage};
...
pub async fn deployer_env() -> DeployerEnv {
    ...
    root.deploy_global_contract(
        DEPLOYER_WITH_USE_ME_WASM.clone(),
    ...
    DeployerEnv {
        sandbox,
        deployer_global_id: GlobalContractId::CodeHash(
            sha256_array(&*DEPLOYER_WITH_USE_ME_WASM).into(),
        ),
    }
}
...
async fn test_deploy_controller_instance(
    ...
    let dummy_account = root
        .generate_subaccount("dummy", NearToken::from_near(1))
        .await
        .unwrap();
    let deployer_code_hash_id = deployer_env.deployer_global_id.clone();

    let storage = DeployerStorage {
    ...
    dummy_account
        .gd_use_me(controller_instance.id())
        .await
    ...
    assert_eq!(
        controller_instance.global_contract_wasm().await.unwrap(),
        *DEPLOYER_WITH_USE_ME_WASM
    );
    ...
    root.gd_use_me(controller_instance.id())
        .await
        .assert_err_contains("MethodNotFound");
}
...
async fn test_deploy_escrow_swap(#[future(awt)] deployer_env: DeployerEnv, unique_index: u32) {
    ...
    let controller_instance = {
        let instance = root
            ...
            .await
            .unwrap();
        root.gd_deploy(instance.id(), &DEPLOYER_WASM).await.unwrap();
        root.gd_use_me(instance.id()).await.unwrap();
        instance
    };
    ...
    root.gd_deploy(escrow_controller_instance.id(), &ESCROW_SWAP_WASM)
        .await
        .unwrap();
...
async fn test_deploy_escrow_instance_on_dummy_wasm_then_upgrade_code_to_escrow_using_controller(
    ...
    root.gd_deploy(escrow_controller_instance.id(), &MT_RECEIVER_STUB_WASM)
        .await
        .unwrap();
    let escrow_instance_params = dummy_escrow_params(root);
    let escrow_instance = {
...
async fn test_transfer_ownership(#[future(awt)] deployer_env: DeployerEnv, unique_index: u32) {
    ...
    bob.gd_deploy(controller_instance.id(), &DEPLOYER_WASM)
        .await
        .assert_err_contains(ERR_UNAUTHORIZED);
    bob.gd_use_me(controller_instance.id())
        .await
        .assert_err_contains(ERR_UNAUTHORIZED);
    ...
    bob.gd_deploy(controller_instance.id(), &DEPLOYER_WASM)
        .await
        .unwrap();
    bob.gd_use_me(controller_instance.id()).await.unwrap();
    bob.gd_transfer_ownership(controller_instance.id(), alice.id())
        .await
        .unwrap();
}
