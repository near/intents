...
use defuse_sandbox::{
    ...
    extensions::{
        deployer_hash_proxy::DeployerHashProxyExt,
        global_deployer::{DeployerExt, DeployerViewExt},
    },
    sandbox,
};
...
async fn test_approve_upgrade_through_proxy_hash_and_execute_upgrade_from_any_account(
    ...
    let hash_proxy_instance = alice
        .deploy_hash_proxy_instance(hash_proxy_global_id.clone(), proxy_state.into())
        .await
        .unwrap();

    bob.hp_exec(hash_proxy_instance.id(), DUMMY_WASM)
        .await
        .assert_err_contains(hash_proxy_error::ERR_MISSING_APPROVAL);

    bob.hp_approve(hash_proxy_instance.id())
        .await
        .assert_err_contains(hash_proxy_error::ERR_UNAUTHORIZED);
