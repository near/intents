...
#[rstest]
#[tokio::test]
async fn record_withdrawal_and_reject_duplicate(#[future(awt)] root: Near) {
    ...
    unauthorized
        .poa_factory_record_withdrawal(poa_factory.contract_id(), w1, withdrawal.clone())
        .await
        .unwrap_err();

    root.poa_factory_record_withdrawal(poa_factory.contract_id(), w1, withdrawal.clone())
        .await
        .unwrap();
...
#[rstest]
#[tokio::test]
async fn update_withdrawal(#[future(awt)] root: Near) {
    let unauthorized = root
    ...
    root.poa_factory_record_withdrawal(poa_factory.contract_id(), w_upd, original.clone())
        .await
        .unwrap();
    ...
    root.poa_factory_update_withdrawal(
        poa_factory.contract_id(),
        w_upd,
        prev_hash,
        updated_hash,
